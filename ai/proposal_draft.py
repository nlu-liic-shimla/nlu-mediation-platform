...
SYSTEM_PROMPT = """
You are an expert legal mediator drafting an initial settlement proposal
...
1. Header: SETTLEMENT PROPOSAL — Round 1
...
"""
    ...
    if is_failed(result):
        print(f"  ⚠️  Proposal draft generation failed — returning template")
        print(f"      Reason: {result.get('reason', 'Unknown error')}")
        return _fallback_template(conflict)

    # If result is not a string try to extract text
...
def _fallback_template(conflict: ConflictExtraction) -> str:
    """
    Fallback template used when AI call fails.
    Gives mediator a starting point to edit manually.
    """
    return f"""SETTLEMENT PROPOSAL — Round 1

This proposal relates to a {conflict.dispute_type.value.replace('_', ' ')} dispute.
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("TEST — Proposal Draft Generation")
    print("="*50)

    draft = generate_proposal_draft(
        conflict=conflict,
        batna_watna=batna_watna,
        mediator_notes="Party A has strong photographic evidence. Party B has repair invoices. Realistic settlement around 30000-35000 INR."
    )
