...
import { Copy, CheckCircle2, AlertTriangle, RefreshCw, Link } from "lucide-react";
...
const NEXT_ACTION = {
  ...
  FIRST_PARTY_SUBMITTED: "One party submitted — generate link for the other party below",
...
/* ── PartyRow ───────────────────────────────────────────── */
function PartyRow({ label, email, submitted, invited, attemptCount, tk, isSmall }) {
  ...
  if (submitted) {
    ...
    badge = { bg: "#fef2f2", color: "#dc2626", text: `❌ Declined (${attemptCount}×)` };
  } else if (declined) {
    badge = { bg: "#fee2e2", color: "#ef4444", text: `⚠ Declined (${attemptCount}/3)` };
  } else if (invited) {
  ...
  return (
    <div style={{
      ...
      gap: isSmall ? 8 : 0, padding: "14px 0",
      borderBottom: `1px solid ${tk.border}`,
    }}>
      <div>
        <div style={{ fontSize: 12, color: tk.sub, marginBottom: 2 }}>{label}</div>
      ...
      <span style={{
        ...
        alignSelf: isSmall ? "flex-start" : "center", flexShrink: 0,
      }}>
        {badge.text}
      </span>
    </div>
...
function AnalysisStatusBlock({ caseId, dark, navigate }) {
  ...
  if (!analysisStatus || ["pending", "processing"].includes(analysisStatus.status)) {
    return (
      <div style={{
    ...
    return (
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 14, color: "#16a34a", fontWeight: 500 }}>Analysis ready</span>
        </div>
        <button
          onClick={() => navigate(`/mediator/cases/${caseId}/analysis`)}
          style={{
            padding: "6px 16px", borderRadius: 7, background: "#16a34a",
...
/* ── InviteLinkModal ────────────────────────────────────── */
function InviteLinkModal({ party, isFirstTime, onClose, onConfirm, loading, tk }) {
  const partyLabel = party === "requesting_party" ? "Requesting Party" : "Against Party";
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: 24, maxWidth: 440, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)",
      }}>
        <h3 style={{ fontSize: 16, fontWeight: 700, color: tk.text, margin: "0 0 8px" }}>
          {isFirstTime ? "Generate Invitation Link" : "Regenerate Invitation Link"}
        </h3>
        ...
        <div style={{
          ...
          <p style={{ margin: 0, fontSize: 13, color: isFirstTime ? "#1e40af" : "#92400e" }}>
            {isFirstTime
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <button
            onClick={onClose}
            style={{
              padding: "8px 20px", borderRadius: 8, border: `1px solid ${tk.border}`,
              background: "transparent", color: tk.text, fontSize: 13,
          ...
          <button
            ...
            style={{
              ...
              background: "#1e40af", color: "#fff", fontSize: 13,
              cursor: loading ? "not-allowed" : "pointer", fontWeight: 600,
              opacity: loading ? 0.7 : 1,
            }}
          >
            {loading ? "Generating..." : isFirstTime ? "Generate Link" : "Yes, Regenerate"}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 13, color: "#dc2626", margin: "0 0 16px", fontWeight: 500 }}>
        ...
        <div style={{
          ...
          <span style={{
            fontSize: 12, color: tk.sub, flex: 1, overflow: "hidden",
        ...
        <button
          ...
          style={{
            ...
            color: tk.text, fontSize: 13, cursor: "pointer", fontWeight: 500,
          }}
        >
          Done
        </button>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 13, color: tk.sub, margin: "0 0 16px" }}>
          The <strong style={{ color: tk.text }}>{partyLabel}</strong> has declined the
          invitation 3 times. Mediation cannot proceed without both parties.
        </p>
        <div style={{
          padding: "12px 14px", borderRadius: 8,
          ...
          <p style={{ margin: 0, fontSize: 13, color: "#92400e" }}>
            Closing this case will mark it as Mediation Failed.
            The requesting party will be notified that mediation was unsuccessful.
...
export default function CaseOverview() {
  ...
  const [partyStatus, setPartyStatus] = useState({
    party_a_email: null, party_a_submitted: false,
    party_b_email: null, party_b_submitted: false,
  });

  // Invitation status — from /invitation-status endpoint
  // Includes: link_generated, accepted, attempt_count per party
  const [invitationStatus, setInvitationStatus] = useState({
    requesting_party: { link_generated: false, accepted: false, attempt_count: 0 },
  ...
  const fetchPartyStatus = useCallback(async (caseId) => {
    try {
      ...
      if (!status.party_a_submitted && !status.party_b_submitted && subs.length > 0) {
  ...
  const handleInviteConfirm = async () => {
    ...
    try {
      const emailForParty = inviteModal.party === "requesting_party"
  ...
  const getButtonLabel = (partyKey) => {
    ...
    if (!status?.link_generated) return { label: "Get Invitation Link", isFirstTime: true };
  ...
  if (error)
    return (
      <MediatorLayout dark={dark} setDark={setDark}>
        <div style={{
          display: "flex", alignItems: "center",
          justifyContent: "center", height: "60vh", color: "#ef4444",
        }}>
          {error}
        </div>
      </MediatorLayout>
    ...
    <MediatorLayout dark={dark} setDark={setDark}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", width: "100%" }}>
        ...
        <button
          ...
          style={{
            background: "none", border: "none", cursor: "pointer", color: tk.sub,
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <h1 style={{
            fontSize: isSmall ? 20 : 24, fontWeight: 700,
            margin: "0 0 12px", color: tk.text, wordBreak: "break-word",
          }}>
            {caseTitle}
          </h1>
        ...
        <div style={{
          ...
          <span style={{ fontSize: 14, color: dark ? "#93c5fd" : "#1e40af", fontWeight: 500 }}>
            {nextAction}
          </span>
        </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <button
                ...
                style={{
                  padding: "6px 16px", borderRadius: 7, background: "#1e40af",
                  color: "#fff", border: "none", cursor: "pointer",
                  fontSize: 12, fontWeight: 600,
                }}
              >
        ...
        {caseData.status === "PROCESSING_FAILED" && (
          <div style={{
            ...
            <button
              ...
              style={{
                padding: "6px 16px", borderRadius: 7, background: "#ef4444",
                color: "#fff", border: "none", cursor: "pointer",
                fontSize: 12, fontWeight: 600,
              }}
            >
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: "#ef4444", marginTop: 2 }}>
                  Mediation cannot proceed without both parties.
                  You may close this case or contact the party directly.
                </div>
              </div>
            </div>
            <button
              onClick={() => setCloseModal({
                partyKey: requestingPartyMaxDeclines ? "requesting_party" : "against_party",
                partyLabel: requestingPartyMaxDeclines ? "Requesting Party" : "Against Party",
              })}
              style={{
                padding: "6px 16px", borderRadius: 7, background: "#ef4444",
                color: "#fff", border: "none", cursor: "pointer",
        ...
        <div style={{
          ...
          <h2 style={{ fontSize: 15, fontWeight: 600, color: tk.text, margin: "0 0 4px" }}>
            Parties
          </h2>
        ...
        {showInvitationSection && (
          <div style={{
            ...
            <p style={{ fontSize: 12, color: tk.sub, margin: "0 0 16px" }}>
              Links are one-time — shown only when generated.
              Use the button below to get a link for each party.
            </p>

            {/* Requesting Party invitation row */}
            {!partyStatus.party_a_submitted && (() => {
              const { label, isFirstTime } = getButtonLabel("requesting_party");
              return (
                <div style={{ marginBottom: 14 }}>
                  <div style={{
                    fontSize: 12, color: tk.sub, marginBottom: 6, fontWeight: 500,
                  }}>
                    Requesting Party
                  </div>
                  <div style={{
                    ...
                    <span style={{ fontSize: 12, color: tk.sub, flex: 1, minWidth: 0 }}>
                      {isFirstTime
                        ? "No link generated yet — click to generate"
                        : "Link previously generated — regenerate to get a new one"}
                    </span>
                    <button
                      onClick={() => setInviteModal({ party: "requesting_party", isFirstTime })}
                      style={{
                    ...
                    >
                      {isFirstTime ? <Link size={11} /> : <RefreshCw size={11} />}
                      {label}
                    </button>
                  </div>
                </div>
              ...
              return (
                <div>
                  ...
                  <div style={{
                    ...
                    <button
                      onClick={() => setInviteModal({ party: "against_party", isFirstTime })}
                      style={{
                        display: "flex", alignItems: "center", gap: 5,
                        padding: "5px 10px", borderRadius: 6,
                        border: `1px solid ${isFirstTime ? "#bfdbfe" : tk.border}`,
        ...
        <div style={{
          background: tk.surface, borderRadius: 12,
          border: `1px solid ${tk.border}`, padding: "16px 20px",
        }}>
          <h2 style={{ fontSize: 15, fontWeight: 600, color: tk.text, margin: "0 0 12px" }}>
            Case Details
          </h2>
            ...
            {[
              ...
              <div
                ...
                style={{
                  ...
                  flexDirection: isSmall ? "column" : "row",
                  gap: isSmall ? 2 : 12, padding: "10px 0",
                  borderBottom: `1px solid ${tk.border}`,
                }}
              >
                <span style={{
                  color: tk.sub, flexShrink: 0,
                  minWidth: isSmall ? "auto" : 140,
                }}>
                  {label}
                </span>
                <span style={{
                  fontWeight: 500, color: tk.text,
