...
export default function Dashboard() {
  ...
  const handleCreateCase = async () => {
    ...
    if (
      !newCaseForm.brief_description.trim() ||
      newCaseForm.brief_description.trim().length < 20
    ) {
      setNewCaseError(
    ...
    try {
      ...
      if (Array.isArray(detail)) {
        setNewCaseError(
          detail.map((e) => `${e.loc[e.loc.length - 1]}: ${e.msg}`).join(", "),
        );
  ...
  const highRiskCount = cases.filter(
    (c) => c.status === "PROCESSING_FAILED",
  ).length;
  const approachingDeadline = cases.filter(
    (c) =>
      c.next_session_date &&
      new Date(c.next_session_date) - Date.now() < 3 * 24 * 60 * 60 * 1000,
  ).length;
  const stalledCount = cases.filter(
    (c) =>
        ...
        {[
          {
            label: "Total Cases",
            value: cases
              .filter((c) => !APPLICATION_STATES.includes(c.status))
              .length.toString(),
            sub: "Active cases",
            subC: "#10b981",
            icon: FileText,
          ...
          >
            ...
            <div
              style={{
                ...
                height: 36,
                borderRadius: 8,
                background: isDark ? "#0f172a" : iconBg,
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
          ...
          >
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewCaseForm((p) => ({ ...p, title: e.target.value }))
                }
                style={{
                  width: "100%",
                  padding: "10px 14px",
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewCaseForm((p) => ({
                    ...p,
                    requesting_party_email: e.target.value,
                  }))
                }
                style={{
                  width: "100%",
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewCaseForm((p) => ({
                    ...p,
                    against_party_email: e.target.value,
                  }))
                }
                style={{
                  width: "100%",
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  color: "#fff",
                  fontSize: 13,
                  cursor: newCaseLoading ? "not-allowed" : "pointer",
                  fontWeight: 600,
                  opacity: newCaseLoading ? 0.7 : 1,
                }}
              >
